...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    if is_main_process and (not is_debug) and use_wandb:
        run = wandb.init(project="Video Color Grading train stage 1", name=folder_name, config=config)
    ...
    train_dataset = Movie_3K_Step1(**train_data)
    ...
    referencenet = DDP(referencenet, device_ids=[local_rank], output_device=local_rank, find_unused_parameters=True)
    ...
    if args.resume is not None and os.path.isfile(args.resume):
        ...
        unet_to_load = unet.module if isinstance(unet, DDP) else unet
        ref_to_load  = referencenet.module if isinstance(referencenet, DDP) else referencenet
        unet_to_load.load_state_dict(ckpt["unet_state_dict"], strict=True)
        ref_to_load.load_state_dict(ckpt["referencenet_state_dict"], strict=True)

        global_step = ckpt.get("global_step", 0)
        saved_epoch = ckpt.get("epoch", 0)

        lr_scheduler.last_epoch = global_step * gradient_accumulation_steps - 1

        first_epoch = saved_epoch
        resume_step_in_epoch = global_step % math.ceil(len(train_dataloader) / max(1, gradient_accumulation_steps))

        if is_main_process:
            logging.info(
                f"Resumed from {args.resume} (epoch={saved_epoch}, "
                f"global_step={global_step}, resume_step_in_epoch={resume_step_in_epoch})"
            )
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            target_frames = batch["target_frames"].to(local_rank)
            ...
            with torch.no_grad():
                target_latents = vae.encode(target_frames).latent_dist
                target_latents = target_latents.sample()
            ...
            input_latents = vae.encode(input_frames).latent_dist
            input_latents = input_latents.sample()
            input_latents = input_latents * 0.18215
            noisy_latents = torch.cat([noisy_latents, input_latents], dim=1)
            ...
            with torch.no_grad():
                ...
                ref_clip, inp_clip = encoder_hidden_states.chunk(2)
                clip_diff = ref_clip - inp_clip

            
            # support cfg train
            mask = drop_image_embeds > 0
            mask = mask.unsqueeze(1).unsqueeze(2).repeat(2, 1, 1).expand_as(encoder_hidden_states)
            ...
            with torch.cuda.amp.autocast(enabled=mixed_precision_training):
                ...
                ref_input = torch.cat([ref_latents, input_latents], dim=0)
                referencenet(ref_input, ref_timesteps, encoder_hidden_states)
                reference_control_reader.update(reference_control_writer)
                
                model_pred = unet(noisy_latents, timesteps, clip_diff).sample
