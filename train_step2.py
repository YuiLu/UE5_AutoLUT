...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    unet = UNet2DConditionModel.from_pretrained(pretrained_sd_path, subfolder="unet", in_channels=6, out_channels=3, low_cpu_mem_usage=False, ignore_mismatched_sizes=True)
    ...
    if args.resume is not None and os.path.isfile(args.resume):
        ...
        model_to_load = unet.module if isinstance(unet, DDP) else unet
        model_to_load.load_state_dict(ckpt["unet_state_dict"], strict=True)
        global_step = ckpt.get("global_step", 0)
        first_epoch = ckpt.get("epoch", 0) + 1
        ...
        print(f"Resumed from checkpoint: {args.resume} "
              f"(epoch {first_epoch}, global step {global_step})")

        if is_main_process:
            logging.info(f"Resumed from {args.resume} (epoch={first_epoch}, global_step={global_step})")
    else:
        global_step = 0
        first_epoch = 0
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with torch.cuda.amp.autocast(enabled=mixed_precision_training):
                ...
                model_pred = unet(noisy_lut, timesteps, clip_diff).sample
